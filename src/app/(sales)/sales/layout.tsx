...
const salesNavItems = [
  { href: "/sales", label: "Sales Home", exact: true },
  { href: "/sales/channels", label: "Channel Breakdown", exact: false },
  { href: "/sales/items", label: "Item Performance", exact: false },
  { href: "/sales/employees", label: "Employees", exact: false },
];
...
function SalesShell({ children }: { children: ReactNode }) {
  const pathname = usePathname();
  const { scope, setScope, options } = useExecutiveScope();
  const { filters, setFilters } = useSalesFilters();

  // Derive active page label for breadcrumb
  const activeNavItem = salesNavItems
    .slice()
    .reverse()
    .find((item) =>
  ...
  return (
    <div className="min-h-screen bg-[#f8f9fb]">
      ...
      <div className="pointer-events-none fixed inset-0 -z-10 overflow-hidden">
        <div className="absolute -top-40 -left-20 h-[400px] w-[400px] rounded-full bg-indigo-200/12 blur-3xl" />
        <div className="absolute top-1/2 -right-20 h-[350px] w-[350px] rounded-full bg-emerald-200/12 blur-3xl" />
      </div>
        ...
        <div className="border-t border-slate-100 px-6">
          <div className="flex items-center gap-1 -mb-px overflow-x-auto">
            ...
            <div className="flex items-center gap-1.5 pr-4 mr-2 border-r border-slate-200 flex-shrink-0 py-3">
              ...
              <span className="text-xs font-bold text-slate-500 uppercase tracking-wider">Sales</span>
            </div>
      ...
      <main className="mx-auto max-w-[1600px] px-6 py-6">
        ...
        <nav className="mb-4 flex items-center gap-1.5 text-xs text-slate-500">
          ...
          <Link href="/sales" className="hover:text-slate-700 transition-colors font-medium">Sales</Link>
          {activeNavItem && activeNavItem.href !== "/sales" && (
            <>
              <ChevronRight size={12} />
